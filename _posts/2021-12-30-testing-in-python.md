...
- They describe what the code should do. So without diving into the code (possibly written by someone else), I can still understand what the code does.
...
Static code tests are tests where the code isn't run, but just the uncompiled code is analyzed. In comparison, on to the other levels the code is actually run. Although static testing often isn't part of the test pyramid, I still think it has some value. There are many different static tests, but for Python linting is the most important one. Since Python allows for [dynamic typing](https://realpython.com/python-type-checking/#dynamic-typing) and there is no go to editor (I prefer VS Code of course), this can lead to interesting code. I learned that you can use multiple linters as long as they don't focus on the same thing. In the next paragraphs I'll be talking about the different linters for Python that I think have value.
